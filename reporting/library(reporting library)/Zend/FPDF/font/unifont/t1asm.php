...
*/
	...
	$this->file_ip = dirname(preg_replace('/\\\\/','/',__FILE__)) . '/' .$fontname;	// This current folder
	$this->lenIV = 4;
	$this->cs_start = 'RD';
...
function DefineChars($chars,$pdfa=false) {
	...
	foreach ($chars AS $c1=>$c2) {	// c1 decimal codepoint => c2 decimal(Unicode)
			...
			if ($pdfa && $op != '.notdef') { $this->pdfa_charset .= "/" . $op; }
			$this->useChars[] = $op;
			$last_c1 = $c1;
	}
}
