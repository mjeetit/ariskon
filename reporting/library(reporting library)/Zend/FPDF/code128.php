...
function PDF_Code128($orientation='P', $unit='mm', $format='label',$height = 374) {

	parent::FPDF($orientation,$unit,$format,$height);
...
function Code128($x,$y,$code,$w,$h) {
        ...
        while ($code > "") {
				...
				//print_r($Cguid);die;
                if (substr($Cguid,0,$IminiC) == $SminiC) { //echo "Hi";die;                                 // jeu C
...
function DPDCode128($x, $y, $code, $w, $h) {
	$Aguid = "";                                                                      // Création des guides de choix ABC
	$Bguid = "";
	$Cguid = "";
	$code=urldecode($code);//Added by nishant
	for ($i=0; $i < strlen($code); $i++) {
		$needle = substr(strtolower($code),$i,1);
		$Aguid .= ((strpos($this->Aset,$needle)===false) ? "N" : "O"); 
		$Bguid .= ((strpos($this->Bset,$needle)===false) ? "N" : "O"); 
		$Cguid .= ((strpos($this->Cset,$needle)===false) ? "N" : "O");
	}
...
function Code39($xpos, $ypos, $code, $baseline = 0.5, $height=5,$text = false) {
...
function Code39_PostNL($x, $y, $code, $ext = true, $cks = false, $w = 0.5, $h = 24, $wide = false) {
    //Display code
    $this->SetFont('Arial', '', 10);
    $this->Text($x+42, $y+$h+4, '*'.$code.'*');
