...
switch($mode){
  ...
  case 'Street':
   ...
   case 'Product':
	 ...
	 foreach($products as $product){
		...
		$data['Price'] = $product['mrp_incl_vat'];
		$newdata[] = $data;
	  }
     break;	
  case 'Expense':
  ...
  case 'Chemist':
   ...
   case 'Employee':
	...
	break;
	case 'PatchList':
	 $getpatchlist = getPatchList($parent);
...
function getDoctors($param=false,$designation,$patch_id) {
   ...
   if($designations['designation_id']==7){
	 ...
	 while($locations = mysql_fetch_assoc($execute)){
	   $city_ids[] = $locations['city_id'];
	   $headquaters[] = $locations['headquater_id'];
	 }
	 $where = " AND PT.city_id IN(".implode(',',$city_ids).") AND PT.headquater_id IN(".implode(',',$headquaters).")";
   }
   if($designations['designation_id']==6){
   ...
   if($designations['designation_id']==8 || $designations['designation_id']==2){
	 ...
	 $execute = mysql_query($sql);
	 $locations = mysql_fetch_assoc($execute);
	 $where = " AND PT.city_id='".$locations['city_id']."' AND PT.headquater_id='".$locations['headquater_id']."'";
   }
   if($param){
     //$where = " AND city_id='".$param."'";
   } 
   $sql = "SELECT DT.doctor_id,DT.doctor_code,DT.doctor_name,DT.street_id,PT.patch_id FROM crm_doctors AS DT INNER JOIN patchcodes AS PT ON PT.patch_id=DT.patch_id WHERE 1".$where;
  // print_r($sql);die;
   $execute = mysql_query($sql);
   $data = array();
   while($result = mysql_fetch_assoc($execute)){
...
function getChemist($param=false,$designation,$patch_id){
   ...
   if($designations['designation_id']==5){
	 ...
	 $execute = mysql_query($sql);
	 $locations = mysql_fetch_assoc($execute);
	 $where = " AND CT.city_id='".$locations['city_id']."'";
   }
   if($designations['designation_id']==7){
     $sql = "SELECT ED.user_id,EL.city_id,EL.headquater_id FROM employee_personaldetail ED INNER JOIN emp_locations EL ON EL.user_id=ED.user_id WHERE ED.parent_id='".$param."'"; 
	 $execute = mysql_query($sql);
   ...
   $sql = "SELECT CT.*,PT.patch_id FROM crm_chemists CT 
		   ...
		   INNER JOIN patchcodes AS PT ON PT.patch_id=DT.patch_id WHERE 1".$where." GROUP BY CT.chemist_id";//print_r($sql);die;
   $execute = mysql_query($sql);
   $data = array();
   while($result = mysql_fetch_assoc($execute)){
   		$data[] = $result;
...
function getPatchList($param){
    $sql = "SELECT PC.patch_id,PC.patchcode,PC.patch_name,PC.city_id,PC.headquater_id FROM emp_locations EL INNER JOIN patchcodes PC ON PC.city_id=EL.city_id AND PC.headquater_id=EL.headquater_id WHERE EL.user_id='".$param."'";
	$execute = mysql_query($sql);
	while($result = mysql_fetch_assoc($execute)){
   		$data[] = $result;
   }
