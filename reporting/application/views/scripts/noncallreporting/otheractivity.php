	 <div class="grid_12">  
	            <!-- Example table -->
                <div class="module">  
                	<h2><span>Other Activity Report</span></h2>
								...
								<td width="20%" align="left" class="bold_text">Activity :
								  <select name="token1">
									...
									<?php foreach($this->doctors as $doctor){
										$select = '';
										if($this->Filterdata['token1']==Class_Encryption::encode($doctor['activity_id'])){
											$select = 'selected="selected"';
										}
									?>
									<option value="<?=Class_Encryption::encode($doctor['activity_id'])?>" <?=$select?>><?=$doctor['activity_name']?></option>
									<?php } ?>
								  </select>
								</td>
