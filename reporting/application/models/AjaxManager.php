...

{
...

	{
		...
		try {
			...
			else if (($_SESSION['AdminLevelID'] != 1 && $_SESSION['AdminLoginID'] != 44) && $_SESSION['AdminDesignation']>$data['designationID']) {
...

	{
		...
		try {
			...
			$query = $this->_db->select()->from('patchcodes','*')->where($where)->where("isActive='1' AND isDelete='0'")->order('patch_name','ASC');
...

	{
		...
		try {
			...
			$query = $this->_db->select()->from('crm_doctors',array('doctor_id','doctor_name'))->where($where)->where("doctor_name!='' AND isActive='1'")->order('doctor_name','ASC'); //echo $query->__toString();die;
...

	{
		...
		try {
			...
			$query = $this->_db->select()->from('crm_stockists','*')->joininner(array('CSH'=>'crm_stockists_hq'),"CSH.stockist_id=crm_stockists.stockist_id",'')->where($where)->where("isActive='1'")->order('stockist_name','ASC');
...

	{
		...
		try {
			...
			$query = $this->_db->select()->from('crm_products','*')->where("isActive='1' AND isDelete='0'")->order('product_name','ASC');
 ...
 public function updateEmpExpense(){
  ...
  foreach($result as $users){
    ...
    if($this->_getData['selected_id']==Class_Encryption::encode($users['user_id'])){

     $selected = 'selected="selected"';

    }

    $string .= '<option value="'.Class_Encryption::encode($users['user_id']).'" '.$selected.'>'.$users['first_name'].' '.$users['last_name'].'</option>';
   ...
   echo $string;exit;  
    ...
    public function getChildhq(){
   $where = '';
   switch($this->_getData['design_id']){
      case '8':
	    $where = 'HQ.headquater_id=EL.headquater_id';
	  break;
	  case '7':
	    $where = 'HQ.area_id=EL.area_id';
	  break;
	  case '6':
	     $where = 'HQ.region_id=EL.region_id';
	  break;
	  case '5':
	     $where = 'HQ.zone_id=EL.zone_id';
	  break;
   }
   if($this->_getData['user_id']>0){
	   $select = $this->_db->select()
			->from(array('HQ'=>'headquater'),array('*'))
			->joininner(array('EL'=>'emp_locations'),$where,array(''))
			->where("EL.user_id='".$this->_getData['user_id']."'")
  ...
  foreach($result as $Headquater){

    $selected = '';

    if($this->_getData['selected_id']==Class_Encryption::encode($Headquater['headquater_id'])){

     $selected = 'selected="selected"';

    }

    $string .= '<option value="'.Class_Encryption::encode($Headquater['headquater_id']).'" '.$selected.'>'.$Headquater['headquater_name'].'</option>';
   ...
   echo $string;exit;  
    ...
    public function getChilDoctor(){
   $where = '';
   switch($this->_getData['design_id']){
      case '8':
	    $where = 'CD.headquater_id=EL.headquater_id';
	  break;
	  case '7':
	    $where = 'CD.area_id=EL.area_id';
	  break;
	  case '6':
	     $where = 'CD.region_id=EL.region_id';
	  break;
	  case '5':
	     $where = 'CD.zone_id=EL.zone_id';
	  break;
   }
   if($this->_getData['user_id']>0){
	   $select = $this->_db->select()
			->from(array('CD'=>'crm_doctors'),array('*'))
			->joininner(array('EL'=>'emp_locations'),$where,array(''))
			->where("EL.user_id='".$this->_getData['user_id']."' AND CD.doctor_name!='' AND CD.isActive='1'")
			->order('CD.doctor_name','ASC');
	}else{
	  $select = $this->_db->select()
        ->from(array('CD'=>'crm_doctors'),array('*'))
		->where("CD.doctor_name!='' AND CD.isActive='1'")
        ->order('CD.doctor_name','ASC');
	}	
        //echo $select->__toString();die;

  $result = $this->getAdapter()->fetchAll($select);
  $string = '<option value="">--Select Doctor--</option>';
  foreach($result as $Headquater){

    $selected = '';

    if($this->_getData['selected_id']==Class_Encryption::encode($Headquater['doctor_id'])){

     $selected = 'selected="selected"';

    }

    $string .= '<option value="'.Class_Encryption::encode($Headquater['doctor_id']).'" '.$selected.'>'.$Headquater['doctor_name'].'</option>';
