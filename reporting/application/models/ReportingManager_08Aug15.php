...

	{
		...
		try {
			...
			if($_SESSION['AdminDesignation']==8){
				...
				$where .= " ED.user_id='".$_SESSION['AdminLoginID']."'";
			...
			elseif($_SESSION['AdminDesignation']==30){
...

	{
		...
		try {
			...
			$select = $this->_db->select()
							   	...
							   	->from(array('EE'=>'app_doctor_visit'),array('DD.doctor_name','DD.speciality','DD.class','CA.activity_name',"DATE_FORMAT(EE.call_date,'%M, %Y') AS visit_month","GROUP_CONCAT(DISTINCT DATE_FORMAT(EE.call_date,'%d') SEPARATOR ', ') AS Days"))
