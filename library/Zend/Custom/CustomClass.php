...
class Zend_Custom_Class extends Zend_Db_Table_Abstract {
            ...
            if($_SESSION['AdminLoginID']==1){
               ...
               $select = $this->_db->select()
                ...
                $select = $this->_db->select()
                                ...
                                ->joininner(array('UP'=>'userprivileges'),'UP.user_id=MT.user_id',array(''))
                                ->where("MT.parent_id='".Bootstrap::$_parent."' AND MT.level_id='".Bootstrap::$_level."'")
                                ->where("MT.user_status='1' AND MT.module_type=1 AND UP.user_id='".$_SESSION['AdminLoginID']."'")
    ...
    **/ 
	...
	public function getSpecialPriv(){
	
    	$select = $this->_db->select()
				->from('user_privillage',array('module_id'))
				->where("user_id='".$_SESSION['AdminLoginID']."'");
		$result = $this->getAdapter()->fetchAll($select);
	
    	$modules = array();
		
        foreach($result as $privilage){
		  $modules[] = $privilage['module_id'];
		}
    ...
    **/
    ...
    /**
    * get Super User Priviledges
    * Function : getSuperUserPrivileges()
    **/
    ...
    /**
    * get Super Sub Module Acess
    * Function : getSuperSubmoduleAccess()
    **/
    ...
    /**
    * get Super Module Action
    * Function : getSuperModuleAction()
    **/    
    ...
    **/   
    public function getUserPrivileges($UserID) {
        ...
        $query = $this->_db->select()
            ...
            ->where('MT.isActive="1"')
            ->where('MT.isDelete="0"')
            ->order("MT.set_order ASC"); 
        //echo $query->__toString();die;

        $allModules = $this->getAdapter()->fetchAll($query); 
        //echo "<pre>";print_r($allModules);echo "</pre>";die;
    ...
    /**
    * get Sub Module Acess
    * Function : getSubmoduleAccess()
    **/
