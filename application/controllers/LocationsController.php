...
class LocationsController extends Zend_Controller_Action {
    ...
    public function patchcodeAction(){
		...
		// Update Patch Name and Fair
		if(!empty($data['uploadpatchfair']) && strtoupper(str_replace(' ','_',$data['uploadpatchfair'])) == 'UPLOAD_&_UPDATE'){
		 	$this->ObjModel->UpdatePatchNameFair($data); 
			$this->_redirect($this->_request->getControllerName().'/'.$this->getRequest()->getActionName());
		}
	...
	public function officeAction(){
	 $this->view->office = $this->ObjModel->getHeadOffice();
	}
