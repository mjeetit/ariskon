...
class Crm_Model_Users extends Zend_Custom

{
   ...
   public function deleteusers(){
	  ...
	  $this->_db->update('employee_personaldetail',array('delete_status'=>'1'),"user_id='".$this->_getData['user_id']."'");
  ...
  public function usersettlementDetail(){
        $select = $this->_db->select()
                                ...
                                ->joinleft(array('EAA'=>'emp_acceseries_account'),"EAA.user_id=EPD.user_id",array('*'))
                                ->where("EPD.employee_code='".$this->_getData['employee_code']."'");//echo $select->__toString();die;
	$userdetails = $this->getAdapter()->fetchAll($select);
      return $userdetails;
  }
