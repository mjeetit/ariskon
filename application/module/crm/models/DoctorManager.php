<?php
	...
	class Crm_Model_DoctorManager extends Zend_Custom

	{
			...
			if(empty($hq_detail)){
			   print_r($hq_name);die;
						...
						$getPatchcity = $this->getLocationData(array('tableName'=>'patchcodes','tableColumn'=>array('city_id',"patch_id"),'columnName'=>'headquater_id','columnValue'=>$hq_detail['headquater_id']));
							...
							$doctorData['headquater_id'] 	= $hq_detail['headquater_id'];
		...
		public function getChemistIdOrAdd_original($chemistName,$data=array())

		{
			...
			$query = $this->_db->select()->from('crm_chemists','chemist_id')->where('patch_id='.$chemistData['patch_id'])->where("UPPER(replace(chemist_name,' ','')) = '".str_replace(' ','',strtoupper($chemistData['chemist_name']))."'")->limit(1); //echo $query->__toString();die;
			...
			if($chemistID<=0) {
				...
				if(trim($chemistData['chemist_name']) != '') {
					...
					$chemistID = ($this->_db->insert('crm_chemists_test',array_filter($chemistData))) ? $this->_db->lastInsertId() : 0;
...

		{
			...
			try{
				...
				$query = $this->_db->select()
								 ...
								 ->joinleft(array('AT'=>'area'),"AT.area_id=HQ.area_id",array('AT.area_name'))
