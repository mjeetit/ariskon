...

{
  ...
  public function __construct(){
	...
	//print_r($this->FromDate);die;
  }

  public function ReCalculationSalary(){
      $select = $this->_db->select()
                                ->from(array('SL'=>'salary_list'),array('*'))
                                ->where("date='".$this->FromDate."' AND salary_processed='0' AND salaryhead_id=3")
								->group("user_id");
	 ...
	 foreach($userslist as $user){
	      $select = $this->_db->select()
				...
				->where("date_format(expense_date,'%Y-%m')< '".date('Y-m',strtotime($user['date']))."' AND approve_status='1' AND salary_status='0' AND user_id='".$user['user_id']."'")
				->group("EE.user_id");
		  $expense  =  $this->getAdapter()->fetchRow($select);
		  if(!empty($expense)){
            $this->_db->update('salary_list',array('amount'=>$user['amount']+$expense['Approve']),"user_id='".$user['user_id']."' AND salaryhead_id=3 AND date='".$this->FromDate."'");
  ...
  public function SalaryList(){
     if(!empty($this->FromDate)){
	     ...
	     $select = $this->_db->select()
							...
							->joinleft(array('DES'=>'designation'),"DES.designation_id=UT.designation_id",array('designation_name','designation_code'))
							->where("UT.user_status='1' AND UT.delete_status='0' AND date_format(UT.doj,'%Y-%m')<='".date('Y-m',strtotime($this->FromDate))."'")
							->order($orderlimit['OrderBy'].' '.$orderlimit['OrderType']);
						    //->limit($orderlimit['Toshow'],$orderlimit['Offset']);
							//echo $select->__toString();die;
	 	$Salaryuserlist = $this->getAdapter()->fetchAll($select);//print_r($Salaryuserlist);die;
	    if(!empty($Salaryuserlist)){
		   $result = array();
		   //$total = 0;
		   foreach($Salaryuserlist as $userdetail){
		      $select = $this->_db->select()
							->from(array('SL'=>'salary_list'),array('COUNT(1) AS CNT','salary_processed'))
							->where("date='".$this->FromDate."' AND (salary_processed='0' OR salary_processed='1') AND user_id='".$userdetail['user_id']."'");
							//echo $select->__toString();die;
		     $salarycount = $this->getAdapter()->fetchRow($select);//print_r($salarycount);die;
			  if($salarycount['CNT']>0){
					if($salarycount['salary_processed']=='0'){
						$result[] =  $userdetail;
						//$total = $total+1;
					 }
			  }else{
			     $this->_getData['user_id'] = $userdetail['user_id'];
					 ...
					 foreach($amountdatas as $salaryamount){
						$providentAmount[$salaryamount['salaryhead_id']] = $salaryamount['amount'];
						 switch($salaryamount['salaryhead_id']){
							case 3:
								$this->getExpenseAmount($userdetail['user_id']);
							  break;
							 default :
							   $this->_db->insert('salary_list',array_filter(array('user_id'=>$userdetail['user_id'],
  ...
  public function GenerateSalary(){
 	...
 	public function CalculateSalary($user_id,$filename){
	    ...
	    *********************************************************************************/
		...
		$userinfo 	= $this->getAllUsersForSalaryHRM($user_id);
		$changectc = 0;
	   ...
	   $this->_salaryData['Deduction'] = array();
	    $this->_salaryData['ExpenseTotal'] = 0;
	     ...
	     switch($salary['salaryheade_type']){
		     case 1:
				...
				if($salary['salaryhead_id']==3){
				  $this->_salaryData['ExpenseTotal'] = $this->_salaryData['ExpenseTotal'] + $salary['amount'];
				}
			    if($salary['salaryhead_id']==7){
				      ...
				      $EsiAmount = $this->ESICalculation($userinfo,$empsalaries,$attandance,$PFAmount);
 ...
 //===================Arear salary Calculation================================   
	$arearsalaries =  $this->getArrierSlalaryAmount($user_id,$this->_salaryDate,true);
	 $mode = '';
	   if(!empty($arearsalaries)){
	     ...
	     switch(trim($arearsalary['salaryheade_type'])){ 
		     case 1:
				 ...
				 if($salary['salaryhead_id']==3){
				  $this->_salaryData['ExpenseTotal'] = $this->_salaryData['ExpenseTotal'] + $arearsalary['amount'];
				}
			    if($arearsalary['salaryhead_id']==7){
				      ...
				      $ArrEsiAmount = $this->ESICalculation($userinfo,$arearsalaries,$Aattandance,$ArrPFAmount);
       ...
       $this->_salaryData['Paid_days'] 	 = (($attandance['total_present']) + ($mode.$Aattandance['total_present']) + ($changectc));
	   ...
	   if(!empty($this->_getData['Type'])){
	       Bootstrap::$LabelObj->outputparam = NULL;
  ...
  public function getSalaryHistory(){
	...
	$orderlimit = CommonFunction::OdrderByAndLimit($this->_getData,'EPD.employee_code');
	$orderlimit1 = CommonFunction::OdrderByAndLimit($this->_getData,'EPD.employee_code','DESC');
	$select = $this->_db->select()
		 				->from(array('SH'=>'salary_history'),array('COUNT(1) AS CNT'))
						->joininner(array('EPD'=>'employee_personaldetail'),"EPD.user_id=SH.user_id",array(''))
						->joininner(array('DES'=>'designation'),"DES.designation_id=EPD.designation_id",array(''))
						->joininner(array('DEP'=>'department'),"DEP.department_id=EPD.department_id",array(''))
     ...
     $select = $this->_db->select()
						...
						->joininner(array('EPD'=>'employee_personaldetail'),"EPD.user_id=SH.user_id",array('CONCAT(first_name," ",last_name) as name','employee_code'))
                        ...
                        ->joinleft(array('EB'=>'emp_bank_account'),"EB.user_id=SH.user_id",array(''))
						->where($where.$filterparam)
						->order("SH.salary_date ".$orderlimit1['OrderType'])
						->order($orderlimit['OrderBy'].' '.$orderlimit['OrderType']);
						//->limit($orderlimit['Toshow'],$orderlimit['Offset']);
  ...
  public function getPreviousHeadSetting(){
   ...
   public function ExportCurrentSalary(){
	  ...
	  $select = $this->_db->select()
						 ...
						 ->where("salaryhead_id<16 AND salary_type=1 AND salaryhead_id NOT IN(3,11,12)")
						 ->order("sequence ASC");
						 //echo $select->__toString();die;
	  $earningheads = $this->getAdapter()->fetchAll($select);
	  foreach($earningheads as $heads){
	     $Header .= "\"" . str_replace( "\"", "\"\"",$heads['salary_title']) . "\"" . $_nxtcol;
	  }
	  $Header .= "\"Total Earning \"".$_nxtcol;
	  $select = $this->_db->select()
	 					 ->from('salary_head',array('*'))
						 ->where("salaryhead_id<16 AND salary_type=2 AND salaryhead_id IN(2,10,15)")
						 ->order("sequence ASC");
						 //echo $select->__toString();die;
	  $deductionhead = $this->getAdapter()->fetchAll($select);
	  foreach($deductionhead as $heads){
	     $Header .= "\"" . str_replace( "\"", "\"\"",$heads['salary_title']) . "\"" . $_nxtcol;
	  }
	  $Header .= "\"Total Deduction \"".$_nxtcol;
	  $Header .= "\"Travel Expense \"".$_nxtcol;
	  $Header .= "\"Net Total \"".$_nxtrow;
	  
	  $select = $this->_db->select()
		 				->from(array('UT'=>'employee_personaldetail'),array('CONCAT(first_name," ",last_name) as name','*'))
						->where("UT.user_status='1' AND UT.delete_status='0'")
						->order('UT.employee_code ASC');
	   $results = $this->getAdapter()->fetchAll($select);
	  //Generate Salary
	  foreach($results as $result){
	    $this->_salaryDate = $this->FromDate;
		$this->_salaryData = array();
		if($this->FromDate==''){
		   $this->_getData['Original'] = true;
		}
		...
		$Header .= "\"" . str_replace( "\"", "\"\"",$this->_salaryData['Paid_days']) . "\"" . $_nxtcol;
		 ...
		 $Header .= "\"" . str_replace( "\"", "\"\"",$this->_salaryData['EarningsTotal']) . "\"" . $_nxtcol; 
		  foreach($deductionhead as $dheads){
		   $Header .= "\"" . str_replace( "\"", "\"\"",$this->_salaryData['Deduction'][$dheads['salaryhead_id']]) . "\"" . $_nxtcol;
	      } 
	  $Header .= "\"" . str_replace( "\"", "\"\"",$this->_salaryData['DeductionsTotal']) . "\"" . $_nxtcol;
	  $Header .= "\"" . str_replace( "\"", "\"\"",$this->_salaryData['ExpenseTotal']) . "\"" . $_nxtcol;
...
//		  	  $finalData[] = $recordData;
  ...
  public function ESICalculation($userinfo,$empsalaries,$attandance,$pfAmount){
	   $earningtotal = 0;
	   $deductiontotal = 0;
	   $grandtotal = 0;
	   ...
	   foreach($empsalaries as $salary){ 
	     switch($salary['salaryheade_type']){
		     case 1:
			    if($salary['salaryhead_id']==1){
				 // $salary['amount'] = $salary['amount'] - $pfAmount['CompanyContPF'];
				  $BASICAMOUNT = $salary['amount'];
				}
			    ...
			    break;
			 case 2:
			 	if($userinfo['provident_pecentage']!='' && $userinfo['provident']==1 && $salary['salaryhead_id']==2){  
					 $salary['amount'] =  $BASICAMOUNT * str_replace('%','',$userinfo['provident_pecentage'])/100;
				 }
				if($salary['prodata_status']=='1' && !in_array($salary['salaryhead_id'],$head_id)){
          ...
          //print_r($earningtotal);die;  
	   $grandtotal = $earningtotal;
		...
		if($userinfo['esi_percentage_comp']!='' && $userinfo['comp_esi_status']==1 && !$this->_salaryData['CompanyCalc']){
				  ...
				  return $this->ESICalculation($userinfo,$empsalaries,$attandance,$pfAmount);
		 }
 ...
 public function ESICalculationNEW($userinfo,$basics,$attandance){
  ...
  public function ExportSalaryBYHead($data){
	   ...
	   $Header .= "\"Employee Code \"".$_nxtcol;
	   $Header .= "\"Employee Name \"".$_nxtcol;
	   $Header .= "\"Headquarter\"".$_nxtcol;
	   $Header .= "\"Department \"".$_nxtcol;
	   $Header .= "\"Paid Days \"".$_nxtcol;
	  ...
	  $select = $this->_db->select()
	 					 ->from(array('SH'=>'salary_history'),array('*'))
						 ->joininner(array('ED'=>'employee_personaldetail'),"SH.user_id=ED.user_id",array('employee_code','CONCAT(ED.first_name," ",ED.last_name) as name'))
						 ->joinleft(array('BK'=>'emp_bank_account'),"SH.user_id=BK.user_id",array(''))
						 ->joininner(array('DEP'=>'department'),"DEP.department_id=ED.department_id",array('DEP.department_name'))
	  ...
	  foreach($results as $salaries){
         if(!empty($salaries['final_salary_encoded'])){
			...
			$Header .= "\"" . str_replace( "\"", "\"\"",$salaries['name']) . "\"" . $_nxtcol;
			$Header .= "\"" . str_replace( "\"", "\"\"",$salaries['headquater_name']) . "\"" . $_nxtcol;
			$Header .= "\"" . str_replace( "\"", "\"\"",$salaries['department_name']) . "\"" . $_nxtcol;
			$Header .= "\"" . str_replace( "\"", "\"\"",$salaries['paid_days']) . "\"" . $_nxtcol;
			...
			foreach($deductionhead as $dheads){
				$Header .= "\"" . str_replace( "\"", "\"\"",$jsonedecode[$dheads['salaryhead_id']]) . "\"" . $_nxtcol;
			} 
			$Header .= "\"" . str_replace( "\"", "\"\"",$salaries['deduction_amount']) . "\"" . $_nxtcol;
			$Header .= "\"" . str_replace( "\"", "\"\"",$jsonedecode[3]) . "\"" . $_nxtcol;
  ...
  public function GenerateCurrentPrevSalary(){
	    ...
	    *********************************************************************************/
	   	...
	   	$userinfo 	= $this->getAllUsersForSalaryHRM($this->_getData['user_id']);
 ...
 //===================Arear salary Calculation================================   
  ...
  public function insertManualSalary(){
  	  $Duretion = $this->getSalaryDuration();
      $date =  date('Y-m-d',mktime(0, 0, 0, date("m",strtotime($this->_getData['date'])), $Duretion['from_date'],  date("Y",strtotime($this->_getData['date']))));
  ...
  public function getManaualArrier($user_id,$slarydate){
       ...
       $select = $this->_db->select()
		...
		if(!empty($result)){
			...
			$mode =  ($result['arr_mode']=='1')?'-':'';
			$attandance['total_salary_days'] = cal_days_in_month(CAL_GREGORIAN,$month,$year);
			$attandance['total_present'] = $result['salary_days'];
			$attandance['arr_mode'] = $result['arr_mode'];
			return $attandance;
		}else{
		    return array();
		}
  ...
  public function insertBunchArrier(){
    $users = array_filter($this->_getData['user_id']);
	$dates = array_filter($this->_getData['date']);
	$salary_dayss = array_filter($this->_getData['salary_days']);
	foreach($users as $key=>$user_id){
	    $this->_getData['date'] = $dates[$key];
		$this->_getData['user_id'] = $user_id;
		$this->_getData['salary_days'] = $salary_dayss[$key];
		$this->_getData['arr_mode'] = 0;//print_r($this->_getData);die;
	    $this->insertManualSalary();
	}
