...
//$basename = 'http://www.ariskon.jclifecare.com';
$basename = 'http://localhost/ariskon/';
?>
...
function getNextRecord1(id,level,target,selected){
$.ajax({
		  url: '<?php echo $basename;?>/Ajax/changestatusrecord',
		  data: 'region_id='+id+'&level='+level+'&selected='+selected,
		  success: function(data) {
		      //alert(data);return false;
			 $("#assign_headquater1").html(data);
		  }
	 });
}

function getNextRecord2(id,level,target,selected){
$.ajax({
		  url: '<?php echo $basename;?>/Ajax/changestatusrecord',
		  data: 'region_id='+id+'&level='+level+'&selected='+selected,
		  success: function(data) {
		      //alert(data);return false;
			 $("#assign_headquater2").html(data);
		  }
	 });
}
...
function changeStatusdesignation(designation_id,selected,sub_desig){ 
    $.ajax({
		  url: '<?php echo $basename;?>/Ajax/changestatus',
		  data: 'Mode=Designation_Child&designation_id='+designation_id+'&selected='+sub_desig,
		  success: function(data) {//alert(data);
			 $("#sub_designation_id").html(data);
		  }
	 });
...
function salarytemplate(user_id){
 ...
 var ctc = $("#ctc").val();
  var radios = document.getElementsByName("heade_type");
  var credit_type;
    for (var i = 0; i < radios.length; i++) {       
        if (radios[i].checked) {
			credit_type = radios[i].value;
        }
    }
  $.ajax({
		  url: '<?php echo $basename;?>/Ajax/salarytemplate',
		  data: 'Mode=Template&bunit_id='+bunit_id+'&department_id='+department_id+'&designation_id='+designation_id+'&user_id='+user_id+'&credit_type='+credit_type+'&ctc='+ctc,
		  success: function(data) {
		       //alert(data);return false;
			 // $("#afterid").after('');
			  $("#template").html(data);
...
function showform(id){
   ...
   for(i=1;i<12;i++){
     if(id==i){
	  ...
	  //Newly inserted Code for User insertion process
	   if($('#table1 thead tr td select#designation_id option:selected').text())
	   {
			val=$('#table1 thead tr td select#designation_id option:selected').text();
			
			if(val=="Area Business Manager"){
				//Only Display if all these were hidde
				if($('#table9 thead #hideReagon').is(':hidden')){
					$('#table9 thead #hideReagon').show();
				}
				if($('#table9 thead #hideArea').is(':hidden')){
					$('#table9 thead #hideArea').show();
				}
				if($('#table9 thead #hideHeadQuater').is(':hidden')){
					$('#table9 thead #hideHeadQuater').show();
				}
			}
			
			else if(val=="Regional Business Manager"){
			
				if($('#table9 thead #hideReagon').is(':hidden')){
					$('#table9 thead #hideReagon').show();
				}
				
				if($('#table9 thead tr td select#area_id option:selected').text()){
					$('#table9 thead #hideArea').hide();
					$('#table9 thead #hideHeadQuater').hide();
				}
			}
			else if(val=="Zonal Business Manager"){
				if($('#table9 thead tr td select#area_id option:selected').text()){
					$('#table9 thead #hideReagon').hide();
					$('#table9 thead #hideArea').hide();
					$('#table9 thead #hideHeadQuater').hide();
				}
			}
...
function getExpenseAmount(head_id){
  $("#fare_amount").hide();
   if($.trim(head_id)=='2' || $.trim(head_id)=='3' || $.trim(head_id)=='4' || $.trim(head_id)=='5' || $.trim(head_id)=='9' || $.trim(head_id)=='11'){
	...
	if($.trim(head_id)=='5'){
	  $("#travel_destination").prop('required',true);
	}
    $.ajax({
		  ...
		  success: function(data) {
		     //alert(data);return false;
			  ...
			  var spliArr = data.split("^");
			  $("#expense_amount").val(spliArr[0]);
			   if($.trim(spliArr[1])=='T'){ 
					$("#mixed").show();
				}else{  
				   $("#mixed").hide();
...
function datecheck(id){
 ...
 if(expensedate>currentdate){
    ...
    $('#dateerror'+id).show();
    $('#headerror'+id).hide();
 }else if($('#head_id'+id).val()=='5' && $('#travel_destination'+id).val()==''){
    alert('Destination can not be blanck In travel expense');return false;
 }else{
