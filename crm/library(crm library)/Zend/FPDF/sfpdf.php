...
*                                                                              *
*******************************************************************************/
function sFPDF($orientation='P', $unit='mm',  $format='label', $height = 374)
{
	//Some checks
	$this->_dochecks();
	...
	$this->CoreFonts=array('courier'=>'Courier', 'courierB'=>'Courier-Bold', 'courierI'=>'Courier-Oblique', 'courierBI'=>'Courier-BoldOblique',
		...
		'symbol'=>'Symbol', 'zapfdingbats'=>'ZapfDingbats','dejavu'=>'dejavu');
	//Scale factor
	if($unit=='pt')
		$this->k=1;
	elseif($unit=='mm')
	...
	$this->PageFormats=array('a3' => array(841.89,1190.55),
                                 ...
                                 'label' => array(258,$this->pageHeight),
                                 'label_gls' => array(290,390), //380
                                 'label_postat' => array(290,430),
                                 'label_bpi' => array(446,290),
                                 'label_bring_short' => array(290,320),
                                 'label_bring' => array(290,380),
                                 'label_hermes' => array(290,375),
                                 //'label_swiss_post' => array(360,340),
                                 'label_swiss_post' => array(290,$this->pageHeight),
                                 //'label_swiss_post' => array(425,$this->pageHeight),
                                 'label_test' => array(500,340));
                                 //'label_swiss_post' => array(485,344)); actulal height -> 374
	if(is_string($format))
	...
	//Page margins (1 cm)
	$margin=0/$this->k; //$margin=28.35/$this->k;
...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if ($uni) {
		include($this->_getfontpath().'unifont/'.$file);
		//var_dump($this->_getfontpath().'unifont/'.$file);
		//exit;
		$type = "Type1subset";
		//$type = "TrueType";
		if(!isset($name)) {
		...
		$this->fonts[$fontkey] = array('i'=>$i, 'type'=>$type, 'name'=>$name, 'desc'=>$desc, 'up'=>$up, 'ut'=>$ut, 'cw'=>$cw, 'enc'=>$enc, 'file'=>$file, 'subsets'=>array(0=>$sbarr), 'subsetfontids'=>array($i), 'used'=>false);
...
function SetFont($family, $style='', $size=0)
{
	...
	if ($this->fonts[$fontkey]['type']=='Type1subset') { $this->unifontSubset = true; }
	...
	foreach($this->fonts as $k=>$font)
	{
		...
		elseif($type=='Type1' || $type=='TrueType')
		{
			...
			$this->_out('endobj');
				   ...
				   $ssfaid="A";
				   if (!class_exists('t1asm')) { include($this->_getfontpath().'unifont/t1asm.php'); }
				   // Create Type1 file using t1asm
				   $asm = new t1asm();
				   $asm->LoadFontFile($this->_getfontpath().'unifont/'.substr($font['file'],0,(strpos($font['file'],'.'))));
				   for($sfid=0;$sfid<count($font['subsetfontids']);$sfid++) {
					...
					$ssfaid++;
					$asm->DefineChars($font['subsets'][$sfid], false);
...
function _putinfo()
{
	$this->_out('/Producer '.$this->_textstring('sFPDF '.sFPDF_VERSION));
	if(!empty($this->title))
...
// Function Added by Sanjeev on 11-Apr-2012 For Text Print Align right
    function CellRight($w, $h=0, $txt='', $border=0, $ln=0, $align='R', $fill=false, $link='')
    {
            //Output a cell
            $k=$this->k;
            if($this->y+$h>$this->PageBreakTrigger && !$this->InHeader && !$this->InFooter && $this->AcceptPageBreak())
